...
function SlicePlane({
  ...
  if (axis === "z") { pos = [0, 0, position]; }
  else if (axis === "y") { rotation = [-Math.PI / 2, 0, 0]; pos = [0, position, 0]; }
  else { rotation = [0, Math.PI / 2, 0]; pos = [position, 0, 0]; }

  return (
    <mesh position={pos} rotation={rotation}>
      <planeGeometry args={[size, size]} />
      <meshBasicMaterial color="#ef4444" side={THREE.DoubleSide} transparent opacity={0.3} />
    </mesh>
  );
}
  ...
  return (
    <div className="w-full h-full bg-[#0a0a0f] rounded-xl overflow-hidden border border-zinc-800">
      <Canvas camera={{ position: [3, 2, 5], fov: 45 }} gl={{ antialias: true }}>
        ...
        <Grid args={[20, 20]} cellSize={1} cellThickness={0.5} cellColor="#27272a"
              sectionSize={5} sectionThickness={1} sectionColor="#18181b"
              fadeDistance={50} infiniteGrid />
        <OrbitControls enableDamping dampingFactor={0.08} />
        {stlUrl && <StlMesh url={stlUrl} onBounds={onBoundsReady} onGeometry={onGeometryReady} />}
        {stlUrl && <SlicePlane axis={sliceAxis} position={slicePosition} visible={showPlane} />}
      </Canvas>
    </div>
  );
}
